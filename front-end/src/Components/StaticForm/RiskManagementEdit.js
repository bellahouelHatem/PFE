...
import cookies from 'js-cookie';
  ...
  const [x,setX]=useState(Questions)
    ...
    function save(e)
     {setformData({...formData,"dateCreation":d,"idInspection":props.location.state.id})
     const token =localStorage.getItem("token");
     const dtoken =jwtDecode(token);
     
      console.log(formData)
     axios.put('http://localhost:8082/api/RiskManagementForm/'+x.id,formData).catch(err=>console.log(err))
    ...
    useEffect(()=>{
      ...
      axios.get('http://localhost:8082/api/RiskManagementFormInsp/'+props.location.state.id).then(resp=>{
                        ...
                        <div sm="6" lg="6" xl="3" id={index} >
                          <Card>
                            ...
                            {qt.resp=="Yes"&&<React.Fragment>
                              <CardBody className="">
                              <FormGroup check>
                                  <Input defaultChecked value="Yes" name={qt.name} type="radio" onChange={(e)=>onChange(e)}/>
                              ...
                              <FormGroup check>
                                  <Input defaultChecked value="NA" name={qt.name} type="radio" onChange={(e)=>onChange(e)}/>
                                  <Label  check>N/A</Label>
                              </FormGroup></CardBody>}
                              {qt.resp==="No"&&
                              <CardBody className="">
                              ...
                              <FormGroup check>
                                  <Input defaultChecked value="No" name={qt.name} type="radio" onChange={(e)=>onChange(e)}/>
                                  <Label check>No</Label>
                              </FormGroup>
                              <FormGroup check>
                                  <Input value="NA" name={qt.name} type="radio" onChange={(e)=>onChange(e)}/>
